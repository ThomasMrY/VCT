...
class VQ_VAE(nn.Module):
    ...
    def __init__(self, hidden=200, k=10, vq_coef=0.2, comit_coef=0.4, emb_out = False, **kwargs):
    ...
    def encode(self, x):
        h1 = self.relu(self.fc1(x))
        h2 = self.fc2(h1)
        im_code = h2.view(-1, self.emb_size, int(self.hidden / self.emb_size))
        if not self.emb_out:
            return im_code
        else:
            z_q = self.emb(im_code, weight_sg=True).view(-1, self.hidden)
            return z_q
...
class Encoder4GAN(nn.Module):
    def __init__(self, d, bn=True, num_channels=3):
        super(Encoder4GAN, self).__init__()
...
class CAE(nn.Module):
    ...
    def forward(self, x):
        z_e = self.encode(x)
        return self.decode(z_e), None, None, None
...
class VQ_CVAE_Mask(nn.Module):
    def __init__(self, d, k=10, bn=True, vq_coef=1, commit_coef=0.5, num_channels=3, **kwargs):
        super(VQ_CVAE_Mask, self).__init__()
        ...
        self.decoder = nn.Sequential(
            ...
            nn.ReLU(inplace=True),
            nn.ConvTranspose2d(
                d0, num_channels+1, kernel_size=4, stride=2, padding=1),
        )
        self.d = d
        self.emb = NearestEmbed(k, d)
        self.vq_coef = vq_coef
    ...
    def decode(self, x, bs):
        x = self.decoder(x)
        recons, masks = unstack_and_split(x, batch_size=bs)
        masks = masks.softmax(dim = 1)
        recon_combined = torch.sum(recons * masks, dim=1)  # Recombine image.
        return recon_combined

    def decode_full(self, x, bs):
        x = self.decoder(x)
        recons, masks = unstack_and_split(x, batch_size=bs)
        masks = masks.softmax(dim = 1)
        recon_combined = torch.sum(recons * masks, dim=1)  # Recombine image.
...
class VQ_CVAE_Mask2(nn.Module):
    def __init__(self, d, k=10, bn=True, vq_coef=1, commit_coef=0.5, num_channels=3, **kwargs):
        super(VQ_CVAE_Mask2, self).__init__()
        ...
        self.decoder = nn.Sequential(
            ...
            nn.ReLU(inplace=True),
            nn.ConvTranspose2d(
                64, num_channels+1, kernel_size=3, stride=1, padding=1),
        )
        self.d = d
        self.emb = NearestEmbed(k, d)
        self.vq_coef = vq_coef
