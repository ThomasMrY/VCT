    ...
    for epoch in range(train_flags.num_epochs):
        for batch_idx, (data, _) in enumerate(train_loader):
            ...
            with torch.no_grad():
                ...
                original_latents = original_latents.reshape(-1,1,original_latents.shape[-1])
                original_pred = decoder_model(original_latents)
                original_pred = original_pred.reshape(original_pred.shape[0], -1, 4,4)
                
                img_org = model.decode(original_pred, bs)
            ...
            with torch.no_grad():
                ...
                swapped_latents = swapped_latents.reshape(-1,1,swapped_latents.shape[-1])
                swapped_pred = decoder_model(swapped_latents)
                swapped_pred = swapped_pred.reshape(swapped_pred.shape[0], -1, 4,4)

                img_swap = model.decode(swapped_pred,bs)
            ...
            model.decoder.train()
            my_latents_new = my_latents.reshape(-1,1,my_latents.shape[-1])
            opred = decoder_model(my_latents_new)
            opred = opred.reshape(opred.shape[0], -1, 4,4)
            im_rec = model.decode(opred,bs)
            im_loss = mse_loss(im_rec, data)
            ...
            if it % train_flags.save_interval == 0 and it != 0:
                ...
                with torch.no_grad():
                    ...
                    if bs > 16:
                        latent1 = my_latents[:16]
                        latent2 = my_latents[16]
                        visual_num = 16
                    else:
                        latent1 = my_latents[:15]
                        latent2 = my_latents[15]
                        visual_num = 15
                    imgs_list = []
                    im_rec, _, masks = model.decode_full(opred,bs)
                    for i in range(5):
                        imgs_list.append(im_rec[i][None,:])
                        imgs_list += [masks[i,j][None,:].repeat(1,3,1,1) for j in range(0,model_args.concepts_num//2)]
                        imgs_list.append(data[i][None,:])
                        imgs_list += [masks[i,j][None,:].repeat(1,3,1,1) for j in range(model_args.concepts_num//2,model_args.concepts_num)]
                    for i in range(model_args.concepts_num):
                        swapped_latent = latent1.clone()
                        swapped_latent[:,i] = latent2[None, i].repeat(visual_num,1)
                        swapped_latent = swapped_latent.reshape(-1,1,swapped_latent.shape[-1])
                        opred = vct_dec(swapped_latent)
                        opred = opred.reshape(opred.shape[0], -1, 4,4)
                        
                        swap_imgs = model.decode(opred, 16)
                        imgs_list.append(swap_imgs)
                    imgs_record = torch.cat([data[:visual_num].detach().cpu(), data[visual_num][None,:,:,:].repeat(visual_num,1,1,1).detach().cpu()] + [rec.detach().cpu() for rec in imgs_list], dim=0)
                    imgs_record = torch.clip((imgs_record+1.)/2, 0.0, 1.0)
                    if imgs_record.shape[1] != 3:
                        imgs_record = imgs_record.repeat(1,3,1,1)
                    imgs_record = make_grid(imgs_record, nrow=visual_num).permute(1, 2, 0)
...
if __name__ == "__main__":
    ...
    time = datetime.datetime.now().strftime("exp_mask2" + (f"_b{meta_args.batch_size}_" if not meta_args.batch_size == -1 else "")  + ("wo_dis_" if meta_args.wo_dis_loss else "") + meta_args.enc_model + "-%Y-%m-%d-%H-%M-%S-%f")
    ...
    vct_enc = VCT_Encoder(z_index_dim = model_args.concepts_num)
    vct_dec = VCT_Decoder(index_num = 256, z_index_dim=4, ce_loss=True)
    ...
    for p in model.emb.parameters():
        p.requires_grad_(False)
    optimizer = optim.Adam(itertools.chain(vct_enc.parameters(),vct_dec.parameters(), model.decoder.parameters()), lr=model_args.lr)
